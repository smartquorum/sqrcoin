...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeSQRcoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSQRcoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeSQRcoinAmount = strAnonymizeSQRcoinAmount.remove(strAnonymizeSQRcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSQRcoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeSQRcoinAmount*COIN) nMaxToAnonymize = nAnonymizeSQRcoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeSQRcoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeSQRcoinAmount));
        strAnonymizeSQRcoinAmount = strAnonymizeSQRcoinAmount.remove(strAnonymizeSQRcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSQRcoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
