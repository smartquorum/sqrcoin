...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSQRcoinAmount"))
        settings.setValue("nAnonymizeSQRcoinAmount", 1000);
    nAnonymizeSQRcoinAmount = settings.value("nAnonymizeSQRcoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeSQRcoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeSQRcoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeSQRcoinAmount:
            return QVariant(nAnonymizeSQRcoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeSQRcoinAmount:
            nAnonymizeSQRcoinAmount = value.toInt();
            settings.setValue("nAnonymizeSQRcoinAmount", nAnonymizeSQRcoinAmount);
            emit AnonymizeSQRcoinAmountChanged(nAnonymizeSQRcoinAmount);
            break;
        default:
            break;
        }
