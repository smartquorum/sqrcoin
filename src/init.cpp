...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: SQRcoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: SQRcoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeSQRcoinamount=<n> " + _("Keep N SQRcoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SQRcoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SQRcoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("SQRcoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "SQRcoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SQRcoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart SQRcoin to complete") << "\n";
    ...
    nAnonymizeSQRcoinAmount = GetArg("-anonymizeSQRcoinamount", 0);
    if(nAnonymizeSQRcoinAmount > 999999) nAnonymizeSQRcoinAmount = 999999;
    if(nAnonymizeSQRcoinAmount < 2) nAnonymizeSQRcoinAmount = 2;
    ...
    LogPrintf("Anonymize SQRcoin Amount %d\n", nAnonymizeSQRcoinAmount);
