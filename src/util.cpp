...
int nAnonymizeSQRcoinAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SQRcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SQRcoin
    // Mac: ~/Library/Application Support/SQRcoin
    // Unix: ~/.SQRcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SQRcoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "SQRcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "SQRcoind.pid"));
